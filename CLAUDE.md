...
13. **`npm run build` auto-patches `importMap.js` via the `postbuild` hook.** The script at [`scripts/ensure-importmap.mjs`](scripts/ensure-importmap.mjs) re-adds `VercelBlobClientUploadHandler` if Payload's regeneration dropped it (happens when `BLOB_READ_WRITE_TOKEN` isn't set at build time — local dev and any Vercel build that doesn't expose it). The script runs automatically on `npm run build`, is idempotent, and is wired into `package.json`. **Do NOT remove the `postbuild` hook.** If you see the admin login blank or the dashboard rendering only background (parchment bg, zero interactive elements), the first thing to check is whether `postbuild` ran — `grep VercelBlob src/app/(payload)/admin/importMap.js` should show 2 matches after every build. The manual `git checkout HEAD -- importMap.js` workaround is still valid as a belt-and-braces fallback if the script ever fails.
