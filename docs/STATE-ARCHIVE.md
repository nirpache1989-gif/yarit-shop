# STATE archive — historical entries (pre-2026-04-11 night)

> This file holds the historical entries that were split off from `docs/STATE.md` during the 2026-04-11 "Copaia rename + final QA" session. Every entry below represents work that was complete, verified, and live on production at the time it was written — the content is preserved verbatim. Newer work lives in `docs/STATE.md` with the most recent "Latest" entry at the top.
>
> Some entries reference the old brand name "Shoresh" (שורש). The project was renamed to "Copaia" (קופאה) on 2026-04-11 — see ADR-020 in `docs/DECISIONS.md`. Historical mentions of Shoresh are **not** rewritten, because they're historical facts at the time the entry was written, not current-state descriptions.
