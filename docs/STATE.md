...
Phases A + B + C + D + E + design polish all complete and deployed.
...
### Known Phase F gap (not a regression)
Product images currently show as empty cards on production because the seed ran against Neon with `BLOB_READ_WRITE_TOKEN` unset, so the 8 Forever photos uploaded to local filesystem instead of Vercel Blob. Fixable in 5 minutes once the client creates the Blob store via the Vercel dashboard — then re-seed with the token set and images become permanent public Blob URLs. See `docs/ENVIRONMENT.md` → "Media storage — Vercel Blob" for the recipe.
