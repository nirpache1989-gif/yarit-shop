...
>
> **Last updated:** 2026-04-11 very late, end of the **Copaia brand rename + catalog replacement + Track D motion polish + Track B admin UX + docs handoff** session. Feature branch `feat/brand-rename` is 5 commits ahead of `main` (which is pinned at `a3b767d`, the prompt-only commit that kicked off the session). **Prod is UNCHANGED** — still serving `8d50bd4` (ADR-019 / Remove-Forever ship) from `https://yarit-shop.vercel.app` via deploy `dpl_EFBBXQ1ZKxrDe2T7ZJTcQTBsJzui`. Nothing pushed or deployed yet — waiting on Yarit's "push" / "deploy" word before fast-forward merging the feature branch to main or triggering Vercel. All code + Neon migration reasoning is in the commits + `docs/STATE.md`.
